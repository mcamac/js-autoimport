      ...
      return _.flatten(
        promises
          ...
          .map((response) => {
            ...
            var allExports = _.flatten(data
              .body
              .filter((astRecord) => (astRecord.type === EXPORT_NAMED || astRecord.type === EXPORT_DEFAULT) &&
                (astRecord.declaration || astRecord.source))
              .map((astRecord) => {
                if (astRecord.type === EXPORT_DEFAULT) {
                  ...
                  if (astRecord.declaration) {
                    ...
                    return {
                      text: astRecord.specifiers[0].exported.name,
                      file: file,
                      type: 'any',
                      def: false
                    };
